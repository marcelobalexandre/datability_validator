class GreaterThanOrEqualToTest < DatabilityValidatorTest
  def test_greater_than_or_equal_to_with_proc
    [@today, @today_as_string, @today_as_localized_string].each do |today|
      SampleModel.validates(
        :date,
        datability: { greater_than_or_equal_to: Proc.new { today } }
      )
  ...
  def test_greater_than_or_equal_to_with_symbol
    [@today, @today_as_string, @today_as_localized_string].each do |today|
      begin
        ...
        SampleModel.validates(
          :date,
          datability: { greater_than_or_equal_to: :another_date }
        )
